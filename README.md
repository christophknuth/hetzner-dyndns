...
- The compose stack is configured to run behind a Traefik ingress proxy. Traefik listens on the external Docker network `http_network` and `DYNDNS_HOST` is used as the host rule for Traefik
...
- `trusted_proxies` controls whose `X-Forwarded-For` header is believed when a client sends no `myip`. The default (loopback plus all private ranges) already covers Docker setups such as Traefik in front of Caddy. Set it explicitly if a reverse proxy in front of you has a **public** address — otherwise its header is ignored and that proxy's own IP would be published as the record.
